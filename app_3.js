...
sio.sockets.on('connection', function(socket) {
	...
	twit.stream('statuses/filter',{ track: ['love','hate'] },function(stream) {
		...
		if (tweet.text.indexOf('love') > -1)
		{
			counts.love += 1;
			counts.total+=1;
			socket.broadcast.emit('love-tweet',{user: tweet.user.screen_name, text: tweet.text});
 		}
 		if (tweet.text.indexOf('hate') > -1)
 		{
			counts.hate += 1;
			counts.total+=1;
			socket.broadcast.emit('hate-tweet',{user: tweet.user.screen_name, text: tweet.text});
		}
